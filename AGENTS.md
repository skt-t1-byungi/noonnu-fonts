...
- **재현성 필수**: 변경 사항을 검증할 수 있는 **구체적인 방법(Command/Script)**을 제시한다.
- **비용 비례**:
  - **Core Logic**: 스크립트/테스트 코드 필수.
  - **Config/Scaffold/Docs**: 단순 실행 커맨드(`cat`, `ls`, `grep`)나 로그 확인으로 대체 가능.
...
- **Practical**: **Sindre-style DX**. Zero-config("It just works"), 설정 최소화, 검증은 단일 커맨드로.
