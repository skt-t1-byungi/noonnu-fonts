...
- 구현 언어: **TypeScript**
- 설계 원칙:
  - **사람의 결정(커먼센스) 우선**: “하나의 폰트(패밀리)” 정의는 운영적으로 모호할 수 있으므로, 최종 SoT는 **결정 + 근거**다.
  - **LLM은 모호성 관리용 보조**: 애매한 케이스의 요약/질문/제안/문장화를 돕되, 자동 판정의 단일 권위가 되지 않는다.
  - **Deterministic은 산출물에만 강제**: JSON 정렬/인덱스 생성/패키징 결과는 입력이 같으면 출력이 같아야 한다.
...
## 8. Complexity Control: "Simplest First"
- **ID/엔티티 최소화**: 필연적 반례(2개 이상)가 나오기 전엔 `id`나 계층을 분리하지 않는다. 기본은 **통합 모델(Flat) + 해시 식별자**.
- **확장 규칙**: "나중을 위해" 미리 쪼개지 않는다. 현재 요구사항만 해결하는 가장 단순한 형태를 취한다.
- **결정성(Determinism)**: 복잡한 로직 대신 **입력이 같으면 출력이 같은** 정렬/규칙을 우선한다.
