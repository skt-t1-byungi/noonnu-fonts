...
## 3) 정규화/통합(품질 기준)
- **사람의 “패밀리 단위 결정”을 1차 진실(SoT)로 저장**한다.
  - 결과물은 “정답 판정 알고리즘”이 아니라, **결정과 근거의 누적/재사용**이다.
- 폰트 파일 내부 메타데이터(name table, OS/2 weight class, Italic 플래그, variable axes 등)는
  - **자동 정답 판정의 재료**가 아니라, 사람 결정을 돕는 **증거(evidence)** 로 취급한다.
- LLM은 다음에만 사용한다.
  - **모호 지점 정리**(무엇이 불명확한가)
  - **결정 제안**(merge/split/keep + 근거 요약 + 리스크/추가로 확인할 것)
  - **결정 기록 문장화**(README/변경 근거에 쓸 텍스트 초안)
- 규칙 기반(deterministic)은 “판정 자체”가 아니라,
  - **산출물(매니페스트/패키지/CSS/인덱스)의 재현성**을 위해 사용한다.
